...
The hero is a greeting beside the name set in the Allura signature font, with a 300px particle portrait that samples `public/images/hero-portrait.jpg` into Canvas 2D and reassembles itself around the pointer. Every link that leaves the site carries the same small `↗` badge, and every jump to GitHub uses one shared handwritten entry set in Kalam, so the same gesture reads identically in the nav, the footer and the observatory. Project titles use the repository names; Chinese use-case descriptions are curated in `profile.projects`; GitHub metadata remains generated by the fetch script.
